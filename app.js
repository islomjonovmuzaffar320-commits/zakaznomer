const VAT_RATE = 0.12;
const SAMPLE_FILE = 'Заказы с товарами - АСОСИЙ (5).xls';
...
function parseFlatTableRows(rows) {
  ...
  rows.slice(headerIndex + 1).forEach((row, offset) => {
    ...
    const orderNumber = getFlatRowOrderNumber(row, rows, headerIndex, columns, orderCol, storeNameCol);
...
function getFlatRowOrderNumber(row, rows, headerIndex, columns, orderCol, storeNameCol) {
  if (orderCol !== -1) return normalizeOrderNumber(row[orderCol]);

  const storeParentOrder = normalizeOrderNumber(columns[storeNameCol]?.parent);
  if (isTenDigitOrderNumber(storeParentOrder)) return storeParentOrder;

  const parentRow = rows[Math.max(0, headerIndex - 1)] || [];
  const sameColumnOrder = normalizeOrderNumber(parentRow[storeNameCol]);
  if (isTenDigitOrderNumber(sameColumnOrder)) return sameColumnOrder;

  return '';
}

function isTenDigitOrderNumber(value) {
